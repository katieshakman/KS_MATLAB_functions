...
for dirIdx = 1:length(dir_list)
    ...
    if ~strcmp(subFold(1,1), '.') % to ignore . and ..
        ...
        hold all; 
        subplot(sub1ax)
        plot(sub1ax,tPts_list{subIdx,1},avgTrace_list{subIdx,1}); 
        subplot(sub2ax)
        plot(sub2ax,tPts_list{subIdx,1},avgTrace_list{subIdx,1}); 
...
saveas(overlayFig, [parentDir,'_overlay_subplots'],'fig');
saveas(overlayFig, [parentDir,'_overlay_subplots'],'jpg');
